...
fn sample_path() -> &'static Path {
    Path::new(env!("CARGO_MANIFEST_DIR")).join("samples/global/wind.keymaps").leak()
}

#[test]
fn parses_real_sample() {
    let f = read_keymap(&sample_path()).expect("应能读取真实样本");
    assert!(
        f.len() >= 500,
    ...
    assert!(
        f.entries.iter().any(|e| e.action.as_deref() == Some("Window.RepeatLastCommand")),
...
fn action_script_counts_match_expected() {
    ...
    let neither = f.entries.iter().filter(|e| e.action.is_none() && e.script.is_none()).count();
    let both = f.entries.iter().filter(|e| e.action.is_some() && e.script.is_some()).count();
